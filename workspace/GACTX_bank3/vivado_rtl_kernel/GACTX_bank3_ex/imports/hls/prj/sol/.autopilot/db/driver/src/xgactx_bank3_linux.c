...
typedef struct {
    ...
    XGactx_bank3_uio_map maps[ MAX_UIO_MAPS ];
} XGactx_bank3_uio_info;

/***************** Variable Definitions **************************************/
static XGactx_bank3_uio_info uio_info;
...
static int uio_info_read_name(XGactx_bank3_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XGactx_bank3_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XGactx_bank3_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XGactx_bank3_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XGactx_bank3_Initialize(XGactx_bank3 *InstancePtr, const char* InstanceName) {
	XGactx_bank3_uio_info *InfoPtr = &uio_info;
